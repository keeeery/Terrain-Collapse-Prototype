    ...
    public sealed class TerrainSampler : MonoBehaviour
    {
        ...
        private void RebuildConnectedChunk(FallingChunk chunk, Bounds bounds)
        {
            ...
            for (int i = 0; i < localCells.Count; i++)
            {
                ...
                if (!targetWorld.IsOccupied(targetCell)) targetWorld.SetCell(targetCell, chunk.CellTypes[i]);
            }
            ...
            int bestCollisionCount)
        {
            ...
            if (!Mathf.Approximately(alignmentError, bestAlignmentError)) return alignmentError < bestAlignmentError;
        ...
        {
            if (!drawSamplingGizmos || targetWorld == null) return;
